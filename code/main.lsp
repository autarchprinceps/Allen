...
(defun add (one two lr globalselector)
  (cond ((equal one 'A)
         (cond ((equal two 'B) (set globalselector (cons (list 'a lr) (symbol-value globalselector))))
               ((equal two 'C) (set globalselector (cons (list 'h lr) (symbol-value globalselector))))
               (T (print "ERROR: wrong parameter name"))
               )
         )
        ((equal one 'B)
         (cond ((equal two 'A) (set globalselector (cons (list 'a (inverse lr)) (symbol-value globalselector))))
