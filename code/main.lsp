...
(defun r (one two lr)
  (cond ((equal one 'A)
         (cond ((equal two 'B) (setq *a* lr))
               ((equal two 'C) (setq *h* lr))
               (T (print "ERROR: wrong parameter name"))
               )
         )
        ((equal one 'B)
         (cond ((equal two 'A) (setq *a* (inverse lr)))
               ((equal two 'C) (setq *g* lr))
               (T (print "ERROR: wrong parameter name"))
               )
         )
        ((equal one 'C)
         (cond ((equal two 'A) (setq *h* (inverse lr)))
               ((equal two 'B) (setq *g* (inverse lr)))
               (T (print "ERROR: wrong parameter name"))
               )
         )
        (T (print "ERROR: wrong parameter name"))
